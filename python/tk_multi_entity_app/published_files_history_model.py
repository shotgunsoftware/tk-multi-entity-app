...
class PublishedFilesHistoryModel(QtCore.QAbstractListModel, ViewItemRolesMixin):
    ...
    def __init__(self, bg_task_manager, parent=None):
        ...
        super(PublishedFilesHistoryModel, self).__init__(parent=parent)
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == PublishedFilesHistoryModel.SG_DATA_ROLE:
            return data

        # TODO move to hook to customize
        if role == PublishedFilesHistoryModel.VIEW_ITEM_THUMBNAIL_ROLE:
            thumbnail_path = data.get("thumbnail_path")
            if not thumbnail_path:
                return QtGui.QPixmap()
            return QtGui.QPixmap(thumbnail_path)

        if role == PublishedFilesHistoryModel.VIEW_ITEM_HEADER_ROLE:
