...
from .framework_qtwidgets import ShotgunFolderWidget, FilterMenuButton
...
from .published_files_model import PublishedFilesModel
from .published_files_proxy_model import PublishedFilesProxyModel
from .published_files_history_model import PublishedFilesHistoryModel
...
class PublishedFilesAppDialog(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.__content_proxy_model = PublishedFilesProxyModel(self)
        self.__content_proxy_model.setDynamicSortFilter(True)
        self.__content_proxy_model.setSortRole(PublishedFilesModel.SORT_ROLE)
        ...
        self._filter_menu.menu_about_to_be_refreshed.connect(
            lambda: self.__filter_panel_overlay.start_spin()
        )
    ...
    def __create_content_delegate(self, thumbnail=False):
        ...
        if thumbnail:
            ...
            delegate.text_role = PublishedFilesModel.VIEW_ITEM_SHORT_TEXT_ROLE
            ...
            delegate.text_role = PublishedFilesModel.VIEW_ITEM_TEXT_ROLE
            delegate.text_padding = ViewItemDelegate.Padding(5, 7, 7, 7)

        # Set the delegate model data roles
        delegate.header_role = PublishedFilesModel.VIEW_ITEM_HEADER_ROLE
        delegate.subtitle_role = PublishedFilesModel.VIEW_ITEM_SUBTITLE_ROLE
        delegate.thumbnail_role = PublishedFilesModel.VIEW_ITEM_THUMBNAIL_ROLE
        delegate.icon_role = PublishedFilesModel.VIEW_ITEM_ICON_ROLE
        # delegate.expand_role = PublishedFilesModel.VIEW_ITEM_EXPAND_ROLE
        delegate.height_role = PublishedFilesModel.VIEW_ITEM_HEIGHT_ROLE
        delegate.loading_role = PublishedFilesModel.VIEW_ITEM_LOADING_ROLE
        delegate.separator_role = PublishedFilesModel.VIEW_ITEM_SEPARATOR_ROLE
    ...
    def __create_history_delegate(self):
        ...
        # Set the delegate model data roles
        delegate.thumbnail_role = PublishedFilesHistoryModel.VIEW_ITEM_THUMBNAIL_ROLE
        delegate.header_role = PublishedFilesHistoryModel.VIEW_ITEM_HEADER_ROLE
        delegate.subtitle_role = PublishedFilesHistoryModel.VIEW_ITEM_SUBTITLE_ROLE
        delegate.text_role = PublishedFilesHistoryModel.VIEW_ITEM_TEXT_ROLE
        delegate.icon_role = PublishedFilesHistoryModel.VIEW_ITEM_ICON_ROLE
        delegate.separator_role = PublishedFilesHistoryModel.VIEW_ITEM_SEPARATOR_ROLE
    ...
    def _on_filter_menu_refreshed(self):
        ...
        if self.__content_model.rowCount() <= 0:
            self.__filter_panel_overlay.show_message("No filter data.")
        elif self._filter_menu.is_empty():
            self.__filter_panel_overlay.show_message("No filters found.")
        else:
            self.__filter_panel_overlay.hide()
