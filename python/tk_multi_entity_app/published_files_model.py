...
class PublishedFilesModel(QtCore.QAbstractListModel, ViewItemRolesMixin):
    """Model for published files."""
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == PublishedFilesModel.SORT_ROLE:
            dt = data.get("created_at")
            timestamp = dt.timestamp()
            return int(timestamp)

        if role == PublishedFilesModel.SG_DATA_ROLE:
            return data

        # TODO move to hook to customize
        if role == PublishedFilesModel.VIEW_ITEM_THUMBNAIL_ROLE:
            thumbnail_path = data.get("thumbnail_path")
            if not thumbnail_path:
                return QtGui.QPixmap()
            return QtGui.QPixmap(thumbnail_path)
        if role == PublishedFilesModel.VIEW_ITEM_HEADER_ROLE:
            name = data.get("name")
            version = data.get("version_number")
            if name and version:
                return f"{name} v{version:03}"
            return ""
        if role == PublishedFilesModel.VIEW_ITEM_TEXT_ROLE:
            return data.get("entity.CustomNonProjectEntity01.sg_type")
        if role == PublishedFilesModel.VIEW_ITEM_SHORT_TEXT_ROLE:
            return data.get("entity.CustomNonProjectEntity01.sg_type")
        if role == PublishedFilesModel.VIEW_ITEM_SUBTITLE_ROLE:
    ...
    def load(
        ...
        order=None,
        only_latest=True,
        published_file_filters=None,
        published_file_fields=None,
    ):
